...
# Grant write permissions to non-root user
# The app writes logs and may create files under conf_files/ at runtime.
...
# Shell form so ${PORT} is resolved at runtime (allows override via env in compose/k8s)
CMD gunicorn -w 4 -b 0.0.0.0:${PORT} \
     --access-logfile - \
     --error-logfile - \
     --forwarded-allow-ips "*" \
     --pythonpath src \
